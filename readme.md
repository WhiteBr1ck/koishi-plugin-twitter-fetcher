...
*   API 与浏览器能识别出原文语言时，若原文与目标语言相同，Google 翻译和 ChatLuna 翻译都会自动跳过；语言未知时仍会正常翻译。
...
*   话题标签搜索必须使用有效的 `auth_token`。插件会先通过 Puppeteer 打开 `x.com`，在内存中取得搜索所需的 `ct0`。
...
*   **v2.1.2**: 原文与目标语言相同时自动跳过 Google 翻译和 ChatLuna 翻译，避免重复翻译和额外请求。
*   **v2.1.1**: 修复部分 ChatLuna 模型返回的换行转义字符和数字型 HTML 实体被原样发送的问题。
